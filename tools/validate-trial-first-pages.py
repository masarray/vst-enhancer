...
def main() -> int:
    ...
    require(landing_parser.translated >= 120, "Compact landing needs at least 120 bilingual elements")
    require(9 <= landing_parser.details <= 11, "Compact landing must keep 9-11 FAQ/disclosure entries")
    ...
    require(len(landing.splitlines()) <= 380, "Landing is no longer compact")

    description = next(
        (item.get("content", "") for item in landing_parser.meta if item.get("name") == "description"),
        "",
    )
    require("365 days" in description, "Landing description must state evaluation duration")
    require("obligation to buy" in description, "Landing description must state no purchase obligation")
    ...
    require("USD 25" not in landing, "Price must remain outside the trial landing")
    require('href="activation/"' in landing, "Landing must link the separate activation page")
    ...
    require("REQUIRED_CHECKOUT_FIELDS" in activation_js, "Activation page must require transaction disclosures")
    require("noopener noreferrer" in activation_js, "Checkout link must use safe external-link rel")
