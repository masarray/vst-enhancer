#!/usr/bin/env python3
"""Validate indexability and canonical SEO signals for the public site."""
...
class Page(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs) -> None:
        ...
        if tag == "html": self.lang = data.get("lang", "").lower()
        elif tag == "title": self._title = True
        elif tag == "h1": self.h1 += 1
        elif tag == "meta":
            name = data.get("name", "").lower()
            if name == "description": self.description = data.get("content", "").strip()
            elif name in {"robots", "googlebot"}: self.robots.append(data.get("content", ""))
            elif data.get("http-equiv", "").lower() == "refresh": self.refresh = True
        elif tag == "link":
            rel = set(data.get("rel", "").lower().split())
            if "canonical" in rel: self.canonical.append(data.get("href", ""))
...
def validate_page(root: Path, url: str, spec: tuple[str, str, str, str]) -> None:
    ...
    require(35 <= len(parser.title.strip()) <= 75, f"title problem: {url}")
    ...
    directives = {part.strip().lower() for part in parser.robots[0].split(",")}
    ...
    require(parser.hreflang == {"en": en_url, "id": id_url, "x-default": en_url}, f"hreflang mismatch: {url}")
    for payload in parser.json_ld:
        if payload: json.loads(payload)
...
def validate_sitemaps(root: Path) -> None:
    ...
    locs = [node.findtext(f"{{{NS}}}loc", "").strip() for node in tree.findall(f"{{{NS}}}url")]
    expected = list(PAGES)
    require(locs == expected and len(locs) == len(set(locs)), "sitemap.xml URL mismatch")
    for node in tree.findall(f"{{{NS}}}url"):
        require(re.fullmatch(r"\d{4}-\d{2}-\d{2}", node.findtext(f"{{{NS}}}lastmod", "")) is not None, "bad lastmod")
    text_urls = [line for line in read(root / "site/sitemap.txt").splitlines() if line]
    ...
    require(f"Sitemap: {ROOT}sitemap.xml" in robots and f"Sitemap: {ROOT}sitemap.txt" in robots, "sitemap discovery missing")


def main() -> int:
    root = Path(__file__).resolve().parents[1]
    try:
        for url, spec in PAGES.items(): validate_page(root, url, spec)
        ...
        return 1
    print("[PASS] All canonical pages are indexable and sitemap signals are consistent.")
    return 0
